...
public class ReportListener implements ITestListener {
	...
	public String screenshot(ITestResult tr) throws IOException {
		...
		Reporter.log("<h4>Repro Steps</h4><br>"
				+ tr.getMethod().getDescription() + "<br>");
		Reporter.log("<a href=" + destination.getAbsolutePath() + ">screenshot</a><br>");
	...
	private void uploadResultsToTestLink(ITestContext testContext) {
		String testLinkHost = testContext.getCurrentXmlTest().getParameter(
				"getTestLinkHost");
		String testLinkProjectName = testContext.getCurrentXmlTest()
				.getParameter("getTestLinkProjectName");
		String testLinkPlanName = testContext.getCurrentXmlTest().getParameter(
				"getTestLinkTestPlanName");
		String testLinkBuildName = testContext.getCurrentXmlTest()
				.getParameter("getTestLinkBuildName");
		...
		if (Objects.nonNull(testLinkHost) && !testLinkHost.isEmpty()
			...
			String devKey = testContext.getCurrentXmlTest().getParameter(
					"getTestLinkApiKey");
			...
			try {
				...
				log.error(
						"Could not establish connection to TestLink or find objects required.",e);
			}
		}
	}
	...
	private void reportResult(TestLinkAPI testLinkApi, TestPlan testPlan,
		...
		for (ITestNGMethod testMethod : testNgMethods) {
			...
			for (String extId : testCaseIds) {
				try {
					// 'null' as a second param returns latest version of test
					// case
	...
	private String[] getTestCaseIds(ITestNGMethod testMethod,
		...
		if (Objects.nonNull(annotation)) {
			// E.g. "MA-451, MA-452" or "MA-451 MA-452" => ["MA-451", "MA-452"]
